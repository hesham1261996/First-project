                        ...
                        <form method="post" action="{{ route('questions.store') }}" enctype="multipart/form-data" autocomplete="off">
                            ...
                            <div class="pl-lg-4">
                                ...
                                <div class="form-group{{ $errors->has('answers') ? ' has-danger' : '' }}">
                                    ...
                                    <textarea name="title" id="input-answers" class="form-control form-control-alternative{{ $errors->has('answers') ? ' is-invalid' : '' }}"
                                        placeholder="{{ __('answers') }}"  required>{{ old('answers') }}</textarea>
                                ...
                                <div class="form-group{{ $errors->has('quize_id') ? ' has-danger' : '' }}">
                                    ...
                                    <select name="quize_id" class="form-control" required>
                                        @foreach (\App\models\quiz::all() as $quize)
                                        <option value="{{$quize->id}}">{{ \Str::limit($quize->name)}}</option>
                                        @endforeach
